const en = {
  // ── Tabs ──────────────────────────────────────────────────────────────────
  tab: {
    scan:      'Scan',
    search:    'Search',
    list:      'List',
    recommend: 'Recommend',
    profile:   'Profile',
  },

  // ── Common ────────────────────────────────────────────────────────────────
  common: {
    save:           'Save',
    cancel:         'Cancel',
    delete:         'Delete',
    confirm:        'Confirm',
    back:           'Back',
    loading:        'Loading...',
    saving:         'Saving...',
    error:          'An error occurred.',
    copied:         'Copied',
    close:          'Close',
    retry:          'Retry',
    empty:          'No items.',
    continue:       'Continue',
    done:           'Done',
    edit:           'Edit',
    editList:       'Edit list',
    add:            'Add',
    clearAll:       'Clear all',
    selected_one:   '{{count}} selected',
    selected_other: '{{count}} selected',
    currentlyActive:'Currently active',
    tapToSelect:    'Tap to select',
    errorTitle:     'Something went wrong',
    errorMessage:   'An unexpected error occurred. Please try again.',
  },

  // ── Accessibility (VoiceOver labels for icon-only controls) ─────────────────
  a11y: {
    back:           'Go back',
    close:          'Close',
    clearSearch:    'Clear search',
    addFavorite:    'Add to favorites',
    removeFavorite: 'Remove from favorites',
    scanGuide:      'Show scanning guide',
    remove:         'Remove',
  },

  // ── Personal info (Profile → 개인정보) ───────────────────────────────────────
  personal: {
    title:   'Personal info',
    section: 'Personal info',
    name:    'Name',
    email:   'Email',
    nickname: {
      menuLabel:       'Community display name',
      menuDescription: 'Set the nickname used in the community.',
      screenTitle:     'Community display name',
      sectionLabel:    'Community display name',
      fieldLabel:      'Display name',
      placeholder:     'Enter a display name (max 30)',
      emptyHint:       "If left blank, you'll appear as 'Anonymous' in the community.",
      saveButton:      'Save',
      savingButton:    'Saving...',
      successMessage:  'Your community display name has been saved.',
      errorTitle:      'Save failed',
    },
  },

  // ── Auth ──────────────────────────────────────────────────────────────────
  auth: {
    tagline:              'Worry less, eat clir',
    continueWithGoogle:   'Continue with Google',
    continueWithApple:    'Continue with Apple',
    appleComingSoon:      'Apple Sign-In coming soon',
    loginFailed:          'Login failed. Please try again.',
    termsPrefix:          'By pressing "Continue with..." you agree to our ',
    termsService:         'Terms of Service',
    termsAnd:             ' and ',
    termsPrivacy:         'Privacy Policy',
    termsAgreement: {
      title:                        "Please agree to Clir's\nterms of service.",
      agreeAll:                     'I agree to all.',
      agreeAllDescription:          'Agreeing to all includes consent to both required and optional items. You may still use the service even if you decline optional items.',
      termsService:                 'Terms of Service',
      privacyPolicy:                'Privacy Policy',
      personalInformation:          'Personal Information Collection & Use',
      healthDisclaimer:             'Health Disclaimer',
      healthDisclaimerDescription:  "The app's analysis is for reference only. Always check product labels before consumption.",
      ageConfirm:                   'I am 13 years of age or older',
      marketing:                    'Marketing Communications',
      dataAnalytics:                'Data Analytics Consent',
      required:                     '[Required]',
      optional:                     '[Optional]',
      agreeButton:                  'Agree',
    },
    signOut:              'Sign out',
    signOutConfirm:       'Are you sure you want to sign out?',
    deleteAccount:        'Delete account',
    deleteAccountTitle:   'Delete your account?',
    deleteAccountWarning: 'This will permanently delete your account, scan history, saved items, and any member profiles. This action cannot be undone.',
    deleteAccountPrompt:  'To confirm, type the phrase below exactly:',
    deleteAccountPhrase:  'delete my account',
    deleteAccountPlaceholder: 'Type the phrase here',
    deleteAccountConfirm: 'Delete',
    deleteAccountFailed:  'Failed to delete account. Please try again.',
  },

  // ── Profile ───────────────────────────────────────────────────────────────
  profile: {
    title:            'Profile',
    myProfile:        'My Profile',
    sensitivity:      'Sensitivity',
    allergyProfile:   'Allergy Sensitivity Settings',
    noAllergens:      'None',
    settingsSection:  'Settings',
    menuDietary:      'Dietary Restrictions',
    menuSensitivity:  'Sensitivity Settings',
    menuPersonalization: 'Personalization',
    menuFamily:       'Family Profiles',
    menuLanguage:     'Language',
    menuSettings:     'Settings',
    menuTermsPrivacy: 'Terms & Privacy',
  },

  // ── Sensitivity ───────────────────────────────────────────────────────────
  sensitivity: {
    title:       'Sensitivity Settings',
    subtitle:    'Choose how strictly the app filters ingredients based on your allergy profile.',
    strictLabel: 'Strict Mode',
    strictDesc:  'Warns you about ingredients that may contain trace amounts of your allergens (may contain labeling).',
    strictBadge: 'Strict',
    normalLabel: 'Normal Mode',
    normalDesc:  'Only warns you about ingredients that are directly included in the product.',
    normalBadge: 'Normal',
  },

  // ── Dietary Restrictions ──────────────────────────────────────────────────
  dietary: {
    title:    'Dietary Restrictions',
    subtitle: 'Select the ingredients you want to avoid. Changes apply to your active profile.',
    tapRemove:'Tap to remove',
  },

  // ── Multi Profile ─────────────────────────────────────────────────────────
  multiProfile: {
    title:        'Multi Profiles',
    subtitle:     'Add profiles and apply their allergy restrictions to scanning.',
    addProfile:   '+ Add Profile',
    emptyHint:    'No multi profiles yet.\nAdd one to get started.',
    badgeMain:    'Main',
    badgeEnabled: 'Applied',
    deleteTitle:  'Delete Profile',
    deleteMsg:    'Are you sure you want to delete "{{name}}"?',
    noAllergens:  'No allergens registered',
    allergenCount_one:   '{{count}} allergen',
    allergenCount_other: '{{count}} allergens',
    strict:       'Strict',
    normal:       'Normal',
  },

  // ── Multi Profile Detail ──────────────────────────────────────────────────
  multiProfileDetail: {
    headerTitle:     'Profile Detail',
    badgeMain:       'Main',
    deleteBtn:       'Delete',
    scanEnabled:     'Active',
    scanDisabled:    'Inactive',
    sectionSensitivity: 'Sensitivity',
    modeLabel:       'Mode',
    strictDesc:      'Warns about ingredients that may contain trace amounts of allergens.',
    normalDesc:      'Only warns about ingredients directly included in the product.',
    sectionAllergy:  'Allergy Profile',
    sectionDietary:  'Dietary Restrictions',
    noAllergens:     'No allergens registered.',
    enableProfile:   'Apply to Scanning',
    disableProfile:  'Disable',
    notFound:        'Profile not found.',
  },

  // ── Multi Profile Add ─────────────────────────────────────────────────────
  multiProfileAdd: {
    stepNameTitle:    "What's this profile's name?",
    stepNameSubtitle: 'Enter a name for this family member\'s profile.',
    namePlaceholder:  'e.g. Mom, Child, etc.',

    stepDietTitle:    'What best describes this profile?',
    stepDietSubtitle: 'Choose the dietary category that applies.',
    dietAllergy:      'Allergy',
    dietAllergyDesc:  'Has food allergies to manage.',
    dietVegetarian:   'Vegetarian',
    dietVegetarianDesc:'Follows a plant-based diet.',
    dietBoth:         'Both',
    dietBothDesc:     'Has allergies and follows a vegetarian diet.',

    stepSeverityTitle:   'How severe is the allergy?',
    stepSeveritySubtitle:'Understanding severity helps recommend safer ingredients.',
    mild:       'Mild',      mildDesc:     'Minor discomfort or skin reactions.',
    moderate:   'Moderate',  moderateDesc: 'Significant symptoms requiring attention.',
    severe:     'Severe',    severeDesc:   'Risk of anaphylaxis or serious reaction.',

    stepReactionTitle:   'When does the reaction happen?',
    stepReactionSubtitle:'This helps filter ingredients more accurately.',
    immediate:    'Immediate',  immediateDesc:  'Symptoms occur within minutes.',
    delayed:      'Delayed',    delayedDesc:    'Symptoms appear hours later.',
    notSure:      'Not sure',   notSureDesc:    'Reaction timing is unclear.',

    stepAllergyIngrTitle:   'Select allergy categories',
    stepAllergyIngrSubtitle:'Choose the ingredient categories this profile needs to avoid.',
    addCategory:  '+ Add Category',
    tapToRemove:  'Tap to remove',
    modalAllergyTitle:   'Add Allergy Categories',
    modalAllergySubtitle:'Select categories to add to the allergy profile.',
    allAdded:     'All categories already added.',

    stepVegTypeTitle:   'What kind of diet do they follow?',
    stepVegTypeSubtitle:'Choose the option that best matches their eating preferences.',

    stepVeganStrictTitle:   'How strict is the vegan diet?',
    stepVeganStrictSubtitle:'Choose the option that best matches what they avoid.',
    strictVegan:    'Strict Vegan',
    strictVeganDesc:'No lecithin / milk sugar / honey / vitamin D3 / Omega-3',
    flexibleVegan:    'Flexible Vegan',
    flexibleVeganDesc:'Try to avoid lecithin / milk sugar / honey / vitamin D3 / Omega-3',

    stepVegIngrSubtitle:'Based on the diet preference, these ingredients will be excluded from recommendations.',
    modalVegTitle:  'Add to your list',
    modalVegSubtitle:'Choose additional categories to avoid.',
    addItem:        '+ Add',
  },

  // ── Signup ────────────────────────────────────────────────────────────────
  signup: {
    labelName:            'Name',
    labelId:              'ID (Email Address)',
    labelPassword:        'Password',
    placeholderFirstName: 'Enter your First Name',
    placeholderLastName:  'Enter your Last Name',
    placeholderEmail:     'Your Email address',
    placeholderPassword:  'Set your Password',
    placeholderConfirm:   'Confirm your Password',
    errorName:            'Please enter your name.',
    errorEmail:           'Please enter a valid email address.',
    errorPasswordLen:     'Password must be at least 8 characters.',
    errorPasswordMatch:   'Passwords do not match.',
  },

  // ── Email Code ────────────────────────────────────────────────────────────
  emailCode: {
    title:        'Check your email\nfor a code',
    subtitle:     'Please enter the verification code sent to your\nemail address',
    errorCode:    'Please enter all 6 digits of the verification code.',
    didntGet:     "Didn't get the code?  ",
    resend:       'Resend Code',
    inTimer:      '  in  ',
    goBack:       'Go back to ',
    tryAnother:   'Try another email',
  },

  // ── Language ──────────────────────────────────────────────────────────────
  language: {
    title:           'Language',
    deviceDetected:  'Device language detected:',
    deviceBadge:     'Device',
  },

  home: {
    tagline: 'Scan, analyze, stay safe',
    hint:    'Tap the center button to scan a product.',
  },

  product: {
    detail:              'Product Detail',
    allIngredients:      'All Ingredients',
    references:          'References',
    alternativeProducts: 'Alternative Products',
    image:               'Image',
    seeMoreDetail:       'see more detail',
    scannedProduct:      'Scanned Product',
    ingredientsToAvoid:  'Ingredients to avoid',
    suspectedAllergens:  'Suspected Allergens',
    riskWarning:         '** This product contains ingredients that may not be suitable for you.',
    severeDisclaimer:    '** For severe allergies, please double-check all ingredients before consuming.',
  },

  scanUi: {
    title:              'Scan',
    history:            'History',
    historyPill:        'Your Scan History',
    historyLoadError:   'Failed to load scan history.',
    historyEmpty:       'No scanned products yet.',
    cameraPermission:   'Camera Access Required',
    grantPermission:    'Grant Permission',
    cameraDescBarcode:  'CLIR needs camera access to scan barcodes and ingredient labels.',
    cameraDescOcr:      'CLIR needs camera access to photograph ingredient labels.',
    retake:             'Retake',
    analyzing:          'Analyzing...',
    tryAgain:           'Try Again',
    barcode:            'BARCODE',
    ocr:                'OCR',
    ocrSubtitle:        'Scan OCR of the product',
    barcodeGuideInstruction:'Center the barcode\nand hold steady',
    ocrGuideInstruction:'Keep the content clear, in focus, and\nfully visible inside the frame.',
    connectionError:    'Connection Error',
    connectionMessage:  'Unable to connect to the server. Please try again later.',
    cameraError:        'Camera is unavailable. Please restart the app or check permissions.',
    productNotFound:    'Product Not Found',
    scanLabelInstead:   'Scan the ingredient label with OCR instead.',
    scanLabel:          'Scan Label',
    recognitionFailed:  'Recognition Failed',
    analysisFailed:     'Analysis Failed',
    clearerPhoto:       'Please take a brighter, clearer photo of the ingredient label.',
    analyzeFailed:      'We could not analyze this ingredient label. Please try again.',
    good:               'Good',
    poor:               'Poor',
    bad:                'Bad',
    goodBang:           'Good!',
    poorBang:           'Poor!',
    badBang:            'Bad!',
    medicalDisclaimer:  'For reference only. Not medical advice or a diagnosis.',
  },
  favoriteUi: {
    add:       'Add to Favorites',
    favorited: 'Favorited',
  },

  profileUi: {
    personalInformation:   'Personal Information',
    allergyProfileTitle:   'Allergy Sensitivity Settings',
    dietaryProfileTitle:   'My Dietary Profile',
    healthProfileTitle:    'My Health Profile',
    sensitivity:           'Sensitivity',
    myAllergy:             'My Allergy',
    preference:            'Preference',
    dietRestriction:       'Diet Restriction',
    dietPreference:        'Diet Preference',
    strictMode:            'Strict Mode',
    normalMode:            'Normal Mode',
    strict:                'Strict',
    normal:                'Normal',
    allergySensitivitySettings:'Allergy Sensitivity Settings',
    vegetarianOption:      'Vegetarian Option',
    vegetarianOptionSubtitle:'Select your vegetarian type.\nThis helps us filter food according to your dietary rules.',
    ingredientsRestrictionProfile:'Ingredients Restriction Profile',
    allergens:             'Allergens',
    vegetarianDiet:        'Vegetarian Diet',
    noAllergensSet:        'No allergens set.',
    noAllergensTap:        'No allergens set - tap to add',
    multiProfiles:         'Multi Profiles',
    profileName:           'Profile Name',
    profileNamePrompt:     'Enter a name for this profile.',
    profileNamePlaceholder:'e.g. Mom, Child, etc.',
    startSurvey:           'Start Survey',
    setActiveProfile:      'Set as Active Profile',
    permissionRequired:    'Permission required',
    photoPermissionProfile:'Photo library access is needed to change the profile picture.',
    photoPermissionMine:   'Photo library access is needed to change your profile picture.',
    enterName:             'Please enter a name.',
    saveFailed:            'Failed to save. Please try again.',
    addAllergy:            'Add Allergy',
    hideAllergySection:    'Hide',
    addDiet:               'Add Diet',
    hideDietSection:       'Hide',
  },

  listUi: {
    title:        'List',
    favoritesPill:'My Favorite Products',
    deleteFailed: 'Delete failed',
    tryAgain:     'Please try again.',
  },

  recommendUi: {
    community:        'Community',
    featured:         'FEATURED',
    featuredSubtitle: 'Scan any product to check ingredients instantly',
    bannerTitle:      'Eat Smart,\nLive Allergy-Free',
    allCategories:    'All categories',
    reorderSections:  'Reorder Sections',
    dragToReorder:    'Drag to reorder sections',
    seeMore:          'See more',
    trending:         'Trending This Week',
    trendingTab:      'Week Trends',
    trendingEmptyCategory: 'No trending products in this category this week.',
    similarPicks:     "Similar Users' Picks",
    similarPicksTab:  'Similar Trends',
    reviewsTitle:     'Reviews',
    similarReviews:   "Similar User's Reviews",
    shareStory:       'Share your story !',
    writeReview:      'Write a review',
    qa:               'Q&A',
    anonymous:        'Anonymous',
    byAuthor:         'by {{author}}',
    magazine:         'Clir Magazine',
    magazineTab:      'Magazine',
    magazineTitle:    'Magazine',
    qaCategoryAll:        'All Categories',
    qaCategoryProduct:    'Product Asking',
    qaCategoryAllergy:    'Allergy',
    qaCategoryVegetarian: 'Vegetarian Diet',
    askQuestion:          'Ask Questions',
    searchTitlePlaceholder:   'Search Title',
    searchProductPlaceholder: 'Search product by name',
    qaTitlePlaceholder:   'Enter a title (max 20 characters)',
    qaBodyPlaceholder:    'Write your question',
    replyPlaceholder:     'Write a reply',
    editTitlePlaceholder: 'Title',
    editBodyPlaceholder:  'Content',
    hoursAgo_one:   '{{count}} hour ago',
    hoursAgo_other: '{{count}} hours ago',
    daysAgo_one:    '{{count}} day ago',
    daysAgo_other:  '{{count}} days ago',
    magazineBookmark: 'Bookmark',
    magazineHoursAgo: '{{count}} hours ago',
    magazineDaysAgo:  '{{count}} days ago',
    guidelineTitle:   'Community Guideline',
    guidelineIntro:   'Q&A is a space to review product ingredients together and help everyone make safer choices.',
    guidelineRuleProduct: 'Include the product name and brand when asking a question.',
    guidelineRuleLabel:   'Share a clear ingredient label photo or OCR result when possible.',
    guidelineRuleContext: 'Allergy and dietary safety can vary depending on each person’s profile.',
    guidelineRuleEmergency: 'For emergencies or severe allergies, always confirm with a medical professional.',
    guidelineFooter:  'The Clir Official Team may add notices or replies when we find incorrect information or risky advice.',
  },

  qaUi: {
    popularQuestions:     'Popular Questions',
    noMatchingQuestions:  'No matching questions.',
    noQuestions:          'No questions yet.',
    noticeTitle:          'Welcome to the Clir Community',
    askTitle:             'Ask Question',
    submit:               'Post',
    chooseCategories:     'Choose Categories',
    relatedProduct:       'Related Product',
    contentSection:       'Post',
    addPhotos:            'Add Photos',
    notice:               'NOTICE',
    answers:              'Answers',
    noAnswers:            'No answers yet. Be the first to reply.',
    send:                 'Send',
    editPost:             'Edit Post',
    editPostCaption:      'Category and attached images cannot be changed.',
    fieldTitle:           'Title',
    fieldContent:         'Content',
    edit:                 'Edit',
    report:               'Report',
    blockUser:            'Block User',
    block:                'Block',
    manageAnswer:         'Manage Answer',
    reportAnswer:         'Report Answer',
    managePost:           'Manage Post',
    reportPost:           'Report Post',
    alertTitle:           'Notice',
    reportReasonPrompt:   'Please briefly describe the reason. We review within 24 hours.',
    reportReasonRequired: 'Please enter a reason for reporting.',
    reportSubmitted:      'Your report has been received. We review within 24 hours.',
    alreadyReported:      'You have already reported this.',
    reportFailed:         'Could not submit the report.',
    blockUserMessage:     "You will no longer see this user's posts and answers.",
    userBlocked:          'User blocked.',
    blockFailed:          'Could not block the user.',
    deletePostTitle:      'Delete Post',
    deletePostMessage:    'Deleting also removes its answers and cannot be undone.',
    deleteAnswerTitle:    'Delete Answer',
    deleteAnswerMessage:  'A deleted answer cannot be restored.',
    titleBodyRequired:    'Please enter both a title and content.',
    titleBodyLimit:       'Title up to 100 and content up to 2000 characters.',
    answerLimit:          'Answers can be up to 1000 characters.',
    postLoadFailed:       'Could not load the post.',
    answerSubmitFailed:   'Could not post your answer.',
    postAlreadyDeleted:   'The post has already been deleted.',
    editFailed:           'Failed to edit.',
    answerAlreadyDeleted: 'The answer has already been deleted.',
    answerEditFailed:     'Failed to edit the answer.',
    deleteFailed:         'Failed to delete.',
    answerDeleteFailed:   'Failed to delete the answer.',
    errCreateFailed:      'Failed to post.',
    errImageTooLarge:     'Images must be 5MB or smaller.',
    errInvalidFileType:   'Only jpg/png/webp images can be uploaded.',
    errProductNotFound:   'Selected product not found. Please select it again.',
    errInvalidInputCreate:'Please check the title, content, and category.',
    errTooManyRequests:   'Too many requests. Please try again later.',
    errServiceUnavailable:'Service is temporarily unavailable. Please try again later.',
    errNetwork:           'Cannot connect to the network.',
    errTimeout:           'The request timed out. Please try again.',
    errForbidden:         'You can only edit or delete your own posts.',
    errPostNotFound:      'The post is already deleted or does not exist.',
    errAnswerNotFound:    'The answer is already deleted or does not exist.',
    errInvalidInput:      'The request is invalid.',
  },

  // ── Survey ───────────────────────────────────────────────────────────────
  survey: {
    landingTitle:       'Please answer\na few quick questions\nto help us get you\nset up.',
    languageTitle:      'App language',
    languageSubtitle:   'This changes the survey and app language.',
    skip:               'Skip this Process',
    dietaryTitle:       'Tell us your\ndietary preferences.',
    dietarySubtitle:    'Select an option that applies so we can\npersonalise your food experience.',
    dietAllergy:        'Allergy',
    dietVegetarian:     'Vegetarian (Vegan)',
    dietBoth:           'Both',
    allergyDocTitle:    'Do you have a diagnosis\nor medical confirmation\nfor your allergy?',
    allergyDocSubtitle: 'If you do, we can help identify ingredients to avoid\nmore accurately.',
    yesDiagnosis:       'Yes, I have.',
    noDiagnosis:        "No, I don't have.",
    severityTitle:      'How severe is your allergy?',
    severitySubtitle:   'Understanding your allergy type helps us recommend safer ingredients for you.',
    mild:               'Mild',
    moderate:           'Moderate',
    severe:             'Severe',
    reactionTitle:      'When does your allergic reaction usually happen?',
    reactionSubtitle:   'Understanding your allergy type helps us recommend safer ingredient filters.',
    immediate:          'Immediate',
    delayed:            'Delayed',
    notSure:            'Not sure',
    ingredientsTitle:   'Select ingredients to avoid.',
    ingredientsSubtitle:'Choose the ingredients related to your allergy so we can personalise your filter settings.',
    add:                '+ Add',
    itemInputPlaceholder:'Type and confirm',
    categoryNamePlaceholder:'Category name',
    selectCategoryTitle:'Select {{category}}',
    selectCategorySubtitle:'Choose {{category}} ingredients to avoid.',
    searchIngredients:  'Search your ingredients',
    uploadTitle:        'Upload your document.',
    uploadSubtitle:     'Upload a medical document or take a photo so we can review it and identify ingredients you may need to avoid.',
    uploadLabel:        'Tap to upload',
    uploadHint:         'PDF or image',
    uploadChangeHint:   'tap to change',
    docDisclaimer:      '** Your document is only used to help set up your allergy preferences.',
    foundTitle:         'Here are the ingredients\nwe found.',
    selectedTitle:      'Here are the ingredients\nyou selected.',
    selectedSubtitle:   'Review the list and confirm the ingredients you want to avoid.',
    docEditSubtitle:    'Edit your allergy ingredients and confirm it.',
    editList:           'Edit your list',
    complete:           'Complete',
    processing:         'Processing...',
    addNewCategories:   '+ Add new Categories',
    addCategoryModalTitle:'Add new Categories',
    addCategoryModalSubtitle:'Choose allergy categories to add.',
    searchCategories:   'Search categories',
    duplicateCategory:  'This category already exists.',
    dietKindTitle:      'What kind of diet\ndo you follow?',
    dietKindSubtitle:   'Choose the option that best matches your eating preferences.',
    veganStrictTitle:   'How strict is your\nvegan diet?',
    veganStrictSubtitle:'Choose the option that best matches what you avoid.',
    strictVegan:        'Strict Vegan',
    strictVeganDesc:    'No lecithin / milk sugar / honey / vitamin D3 / Omega-3',
    flexibleVegan:      'Flexible Vegan',
    flexibleVeganDesc:  'Try to avoid lecithin / milk sugar / honey / vitamin D3 / Omega-3',
    dietConfirmTitle:   'Your diet preference is',
    dietConfirmSubtitle:'Your selected diet preference will be applied\nto your filtering options.',
    vegetarianAvoidTitle:'As {{diet}},\nyou avoid',
    vegetarianAvoidSubtitle:'Based on your diet preference, these ingredients\nwill be filtered out.',
    addToListTitle:     'Add to your list',
    addToListSubtitle:  'Choose additional categories to avoid.',
    allCategoriesAdded: 'All categories are already added.',
    vegetarianTypes: {
      fruitarian:          'Fruitarian',
      vegan:                'Vegan',
      lacto_vegetarian:     'Lacto-vegetarian',
      ovo_vegetarian:       'Ovo-vegetarian',
      lacto_ovo_vegetarian: 'Lacto-ovo-vegetarian',
      pesco_vegetarian:     'Pesco-vegetarian',
      pollo_vegetarian:     'Pollo-vegetarian',
      flexitarian:          'Flextarian',
    },
    dietTitles: {
      fruitarian:          'Fruitarian',
      vegan:                'Vegan',
      lacto_vegetarian:     'Lacto-Vegetarian',
      ovo_vegetarian:       'Ovo-Vegetarian',
      lacto_ovo_vegetarian: 'Lacto-ovo-Vegetarian',
      pesco_vegetarian:     'Pesco-Vegetarian',
      pollo_vegetarian:     'Pollo-Vegetarian',
      flexitarian:          'Flexitarian',
      strict:               'Strict Vegan',
      flexible:             'Flexible Vegan',
    },
    dietCategories: {
      fruitsGrains: 'Fruits / Grains',
      vegetables:   'Vegetables',
      dairy:        'Dairy',
      eggs:         'Eggs',
      seafood:      'Seafood',
      poultry:      'Poultry',
      redMeat:      'Red Meat',
    },
    dietTypeDescriptions: {
      fruitarian:           '✓  Fruits, grains, nuts\n✗  Vegetables, meat, dairy, eggs, seafood',
      vegan:                '✓  All plant-based foods\n✗  Meat, seafood, dairy, eggs, honey',
      lacto_vegetarian:     '✓  Vegetables, dairy\n✗  Meat, seafood, eggs',
      ovo_vegetarian:       '✓  Vegetables, eggs\n✗  Meat, seafood, dairy',
      lacto_ovo_vegetarian: '✓  Vegetables, dairy, eggs\n✗  Meat, seafood',
      pesco_vegetarian:     '✓  Vegetables, fish & seafood\n✗  Meat, poultry',
      pollo_vegetarian:     '✓  Vegetables, fish & seafood, poultry\n✗  Red meat (beef, pork, lamb)',
      flexitarian:          '✓  Mostly plant-based\n✗  Nothing strictly — just minimizes meat',
    },
    dietCategoryDescriptions: {
      fruitsGrains: 'Includes fruits, grains, cereals, bread, pasta, rice, and other plant-based staples.',
      vegetables:   'Includes all vegetables — leafy greens, root vegetables, legumes, and more.',
      dairy:        'Includes milk, cheese, yogurt, butter, and all products derived from animal milk.',
      eggs:         'Includes chicken eggs and any products that contain eggs.',
      seafood:      'Includes fish, shellfish, crustaceans, and all other aquatic animals.',
      poultry:      'Includes chicken, turkey, duck, and other birds raised for food.',
      redMeat:      'Includes beef, pork, lamb, and other red meats from mammals.',
    },
    // ── SurveyLanding beta invite + notices (F8 §5 / closed-beta) ────────────
    inviteTitle:        'Beta Invite Code',
    inviteDesc:         'This is a closed beta. Please enter the invite code we sent to your email.',
    invitePlaceholder:  'Enter invite code',
    inviteVerify:       'Verify code',
    inviteSuccess:      "Code verified! You're in.",
    inviteErrorTooShort:'Code must be at least 6 characters.',
    inviteErrorInvalid: 'Invalid invite code. Check your email and try again.',
    inviteErrorGeneric: 'Could not verify code. Please try again.',
    noticeNutrition:    'Currently we focus on allergen and dietary preference matching only — we do not measure nutrition values (sodium, sugar, calories) yet. Nutrition tracking coming in a future update.',
    noticeMedical:      'This app is for informational purposes only and is not medical advice. Always consult your doctor for medical decisions. Allergen labeling is based on FDA Big 9 + Sesame; verify the label yourself before consuming.',
  },

  // ── Search ────────────────────────────────────────────────────────────────
  search: {
    placeholder:    'Search Products',
    sortBy:         'Sorted by',
    sortDefault:    'Default',
    sortAlphabetical:'Alphabetical A-Z',
    sortRelevance:  'Relevance',
    sortSafety:     'Safety',
    sortName:       'Name',
    recentTitle:    'Recent Searches',
    empty:          'No results found',
    loading:        'Searching...',
    filters:        'Filters',
    categories:     'Products Categories',
    categoriesList: {
      beverages:  'Beverages',
      snacks:     'Snacks & Chips',
      bakery:     'Bakery & Bread',
      dairy:      'Dairy Products',
      cereals:    'Cereals & Grains',
      cookies:    'Cookies & Crackers',
      chocolates: 'Chocolates & Candy',
      spreads:    'Spreads & Jams',
      meat:       'Meat & Poultry',
      seafood:    'Seafood',
      condiments: 'Sauces & Condiments',
    },
    safeOnlyLabel:  'Show only safe products for me',
    priceRange:     'Price Range',
    minPrice:       'Min Price',
    maxPrice:       'Max Price',
    apply:          'Apply Filters',
    reset:          'Reset',
  },

  // ── Personal Name / Password ──────────────────────────────────────────────
  personalName: {
    title:                  'Edit Profile',
    sectionName:            'Name',
    firstName:              'First name',
    lastName:               'Last name',
    emailAddress:           'Email Address',
    saveName:               'Save Name',
    sectionPassword:        'Change Password',
    currentPassword:        'Current Password',
    newPassword:            'New Password',
    confirmPassword:        'Confirm New Password',
    savePassword:           'Change Password',
    placeholderName:        'Enter your name',
    placeholderCurrentPw:   'Current password',
    placeholderNewPw:       'New password (min. 8 characters)',
    placeholderConfirmPw:   'Confirm new password',
    successName:            'Name updated.',
    successPassword:        'Password changed successfully.',
    errorNameEmpty:         'Name cannot be empty.',
    errorCurrentPw:         'Please enter your current password.',
    errorPwLen:             'Password must be at least 8 characters.',
    errorPwMatch:           'Passwords do not match.',
  },
  // ── Favorites ─────────────────────────────────────────────────────────────
  favorites: {
    title:        'Favorite Products',
    empty:        'No favorites yet.',
    errorLoad:    'Failed to load favorites.',
    retry:        'Try again',
  },
} as const;

export default en;
export type TranslationKeys = typeof en;
