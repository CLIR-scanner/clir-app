...
type Props = {
  navigation: NativeStackNavigationProp<ListStackParamList, 'FavoritesAll'>;
};

export default function FavoritesAllScreen({ navigation }: Props) {
  ...
  const handleRemove = (item: FavoriteItem) => {
    Alert.alert(
      '즐겨찾기 삭제',
      `"${item.product.name}"을(를) 삭제하시겠습니까?`,
      [
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>전체 즐겨찾기 ({favorites.length})</Text>
        <View style={{ width: 52 }} />
      </View>
