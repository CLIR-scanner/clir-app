...
type Props = {
  navigation: NativeStackNavigationProp<RecommendStackParamList, 'SimilarUsersFavorites'>;
};
...
export default function SimilarUsersFavoritesScreen({ navigation }: Props) {
  ...
  useEffect(() => {
    let cancelled = false;
    getSimilarUsersFavorites(currentUserId)
      ...
      <View style={styles.card}>
        ...
        <View style={styles.productInfo}>
          ...
          {item.riskIngredients.length > 0 && (
            <Text style={styles.riskNote} numberOfLines={1}>
              ⚠ {item.riskIngredients.map(i => i.nameKo).join(', ')}
            </Text>
          )}
        </View>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{Strings.recommendSimilarUsers}</Text>
        <View style={{ width: 52 }} />
      </View>
        ...
        <FlatList
          ...
          ListHeaderComponent={
            <Text style={styles.subtitle}>
              비슷한 알러지 프로필을 가진 사용자들이 즐겨찾는 제품입니다
            </Text>
          }
        />
      )}
...
const styles = StyleSheet.create({
  ...
  productBrand: { fontSize: 12, color: Colors.textSecondary, marginBottom: 2 },
  riskNote: { fontSize: 11, color: Colors.caution },
